...
"""Pure NumPy port of the upstream SRU height-field generator.
...
The feature-generation functions below intentionally remain mechanically close
to the IsaacLab source. Platform integration and the 301-by-301 collision border
live in :mod:`training.terrains.atlas`.
"""
...
from .constants import HEIGHTS, PADDING, STAIRS, OBSTACLES, ObstacleType


@dataclass
class SruSubTerrainConfig:
    """Runtime equivalent of the upstream ``HfMazeTerrainCfg`` fields used."""

    size: tuple[float, float] = (29.9, 29.9)
    ...
    add_goal: bool = True
    add_noise_to_flat: bool = False
    randomize_wall: bool = True
    ...
    # Applies only to random walls inside the stairs family. Stair/platform
    # placement remains controlled by the unscaled terrain difficulty.
    stair_obstacle_density_scale: float = 1.0
    stair_height_scale: float = 1.0
    pit_probability: float = 0.15
    open_probability: float = 0.9
    rng: np.random.Generator | None = None
    height_field_visual: Any = None
    height_field_valid_mask: Any = None
    height_field_platform_mask: Any = None
    height_field_spawn_mask: Any = None
...
def make_random_obstacle(
    ...
    pit_probability: float | None = None,
) -> np.ndarray:
    ...
    if is_pit is None:
        probability = (
            OBSTACLES.DEFAULT_PIT_PROB
            if pit_probability is None
            else float(pit_probability)
        )
        is_pit = rng.random() < probability
...
class StairGenerator:
    ...
    STEP_STRUCTURE_PROB = 0.6
    """Probability of generating stepped stairs instead of lowered flat platforms."""

    # A single connected 3x3 structure avoids the two ground-height notches in
    # the old sparse layout. Each approach is a three-cell-wide staircase, so
    # wheel pairs cannot drop into a gap between independently placed blocks.
    LAYOUTS = [
        {
            "platforms": [(1, 0), (1, 1), (1, 2)],
            "stairs": [
                (0, 0, "e"),
                (0, 1, "e"),
                (0, 2, "e"),
    ...
    def __init__(self, wall_height: float, vertical_scale: float, height_scale: float = 1.0):
        self.wall_height = wall_height
        self.height_scale = float(height_scale)
        self.step_platform_height = int(round(HEIGHTS.PLATFORM * self.height_scale))
        self.ramp_platform_height = int(round(HEIGHTS.RAMP_PLATFORM * self.height_scale))
        self.vertical_scale = vertical_scale
        self._make_structure_templates()

    def _make_structure_templates(self):
        """Create both stepped-stair and lowered-platform templates for each direction."""
        cell_px = STAIRS.SINGLE_CELL_PIXELS
        step_bounds = np.rint(np.linspace(0, cell_px, STAIRS.NUM_STEPS + 1)).astype(int)  # 用离散边界切出台阶，像素切分保持与当前台阶级数一致。

        step_template = np.zeros((cell_px, cell_px), dtype=np.int16)
        for i in range(STAIRS.NUM_STEPS):
            start, end = step_bounds[i], step_bounds[i + 1]
            height_value = int(
                min(
                    self.step_platform_height,
                    round(
                        STAIRS.STEP_HEIGHT_METERS
                        * self.height_scale
                        * (i + 1)
                        / self.vertical_scale
                    ),
            ...
            step_template[start:end, :] = height_value

        ramp_template = np.full((cell_px, cell_px), self.ramp_platform_height, dtype=np.int16)  # 原先的斜坡支路改成整格等高的低平台，不再生成连续斜面。

        self.step_templates = self._make_directional_templates(step_template)
        self.ramp_templates = self._make_directional_templates(ramp_template)

    @staticmethod
    def _make_directional_templates(template: np.ndarray) -> dict[str, np.ndarray]:
        """Create directional templates without interpolation artifacts."""
        return {
            "e": template.copy(),
            "n": np.rot90(template, 1).copy(),  # 只保留原来的斜角楼梯布局，方向切换仍用离散 90 度旋转。
            "w": np.rot90(template, 2).copy(),
            "s": np.rot90(template, 3).copy(),
        }
    ...
    def generate(self, rng: np.random.Generator) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        cell_px = STAIRS.SINGLE_CELL_PIXELS
        use_step_structure = rng.random() < self.STEP_STRUCTURE_PROB
        selected_templates = self.step_templates if use_step_structure else self.ramp_templates  # stairs 子地形现在保留 60% 台阶、40% 低平台结构。
        selected_platform_height = self.step_platform_height if use_step_structure else self.ramp_platform_height  # 低平台分支被选中时，连通平台也同步降到较低高度，保持整块结构等高。
        ...
        for gx, gy in layout["platforms"]:
            ...
            heights[xs:xe, ys:ye] = selected_platform_height
        ...
        for gx, gy, direction in layout["stairs"]:
            xs, xe = gx * cell_px, (gx + 1) * cell_px
            ys, ye = gy * cell_px, (gy + 1) * cell_px
            heights[xs:xe, ys:ye] = selected_templates[direction]
...
def _get_rng(cfg: SruSubTerrainConfig) -> np.random.Generator:
...
def maze_terrain(difficulty: float, cfg: SruSubTerrainConfig) -> np.ndarray:
    ...
    stair_gen = StairGenerator(
        wall_height,
        cfg.vertical_scale,
        height_scale=cfg.stair_height_scale,
    )

    # Keep the normal mask/padding pipeline below while suppressing all
    # geometry. This makes Stage 1 use the same sampling contract as later
    # stages without introducing a separate flat-terrain implementation.
    effective_difficulty = 0.0 if cfg.flat_only else difficulty * cfg.obstacle_density_scale

    # Generate base pattern
    if cfg.flat_only:
        maze = np.zeros(cfg.grid_size, dtype=np.uint8)
    elif cfg.non_maze_terrain:
        maze = np.zeros(cfg.grid_size, dtype=np.uint8)
        obstacle_prob = effective_difficulty * OBSTACLES.NON_MAZE_DENSITY
    ...
    if cfg.flat_only:
        pass
    elif cfg.dynamic_obstacles:
        _add_pits(rng, terrain, cfg, effective_difficulty, wall_height, cell_pixels)
    elif cfg.stairs:
        _add_stairs(rng, terrain, cfg, effective_difficulty, wall_height, cell_pixels, stair_gen)
    else:
    ...
    # Keep the upstream config-side output contract, using NumPy instead of
    # Torch because this module must not depend on IsaacLab or a GPU runtime.
    if cfg.add_goal:
        cfg.height_field_visual = terrain.heights.copy()[None, ...]
        cfg.height_field_valid_mask = terrain.valid_mask.copy()[None, ...]
        cfg.height_field_platform_mask = terrain.platform_mask.copy()[None, ...]
        cfg.height_field_spawn_mask = spawn_mask.copy()[None, ...]
    ...
    for x in range(cfg.grid_size[0]):
        for y in range(cfg.grid_size[1]):
            ...
            if cfg.randomize_wall and rng.random() < cfg.random_wall_ratio:
                obs = make_random_obstacle(
                    rng,
                    cell_pixels,
                    wall_height,
                    pillar_weight=pillar_weight,
                    pit_probability=cfg.pit_probability,
                )
...
def _add_stairs(
    ...
    obstacle_prob = (
        difficulty
        * OBSTACLES.STAIRS_OBSTACLE_DENSITY
        * cfg.stair_obstacle_density_scale
    )
