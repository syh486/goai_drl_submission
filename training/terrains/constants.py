...
- PLATFORM: derived from the configured stair total rise (valid for goals)
...
- Platform: centered on the configured platform height with a narrow margin
...
DEFAULT_STAIR_NUM_STEPS: int = 5
"""Number of steps per staircase used by the current terrain setup."""

DEFAULT_STAIR_STEP_HEIGHT_METERS: float = 0.2
"""Original per-step height used by the upstream terrain design."""

STAIR_HEIGHT_SCALE: float = 0.7
"""Global scale applied to all staircase rises for MX/MC friendly terrain."""  # 当前楼梯单级高度保持为 20cm * scale。

STAIR_STEP_HEIGHT_METERS: float = DEFAULT_STAIR_STEP_HEIGHT_METERS * STAIR_HEIGHT_SCALE
"""Effective per-step height after scaling."""

STAIR_PLATFORM_HEIGHT_METERS: float = DEFAULT_STAIR_NUM_STEPS * STAIR_STEP_HEIGHT_METERS
"""Top platform height that matches the total staircase rise."""

STAIR_PLATFORM_HEIGHT_UNITS: int = int(round(STAIR_PLATFORM_HEIGHT_METERS / VERTICAL_SCALE))
"""Top platform height in discretized height-field units."""

RAMP_HEIGHT_RATIO_TO_STAIRS: float = 0.4
"""Relative top-height ratio applied only to the lowered platform structures inside the stairs terrain."""  # 原先 40% 的斜坡分支改为低平台分支，其顶高固定为当前楼梯总高度的 0.4。

RAMP_PLATFORM_HEIGHT_METERS: float = STAIR_PLATFORM_HEIGHT_METERS * RAMP_HEIGHT_RATIO_TO_STAIRS
"""Top platform height for the lowered-platform structures."""

RAMP_PLATFORM_HEIGHT_UNITS: int = int(round(RAMP_PLATFORM_HEIGHT_METERS / VERTICAL_SCALE))
"""Top lowered-platform height in discretized height-field units."""

PLATFORM_HEIGHT_MARGIN_UNITS: int = 5
"""Strict sampling margin around the stair platform height."""

PLATFORM_EDGE_MARGIN_UNITS: int = 20
"""Wider edge-detection margin around the stair platform height."""
...
class HeightValues:
    ...
    # Platform height equals the total staircase rise.
    PLATFORM: int = STAIR_PLATFORM_HEIGHT_UNITS

    # Lowered-platform height used by the non-stair 40% branch.
    RAMP_PLATFORM: int = RAMP_PLATFORM_HEIGHT_UNITS
    ...
    @property
    def ramp_platform_meters(self) -> float:
        """Ramp platform height in meters."""
        return self.RAMP_PLATFORM * VERTICAL_SCALE
...
class SamplingThresholds:
    ...
    # Platform range centered on the configured stair platform height.
    PLATFORM_MIN: int = STAIR_PLATFORM_HEIGHT_UNITS - PLATFORM_HEIGHT_MARGIN_UNITS
    PLATFORM_MAX: int = STAIR_PLATFORM_HEIGHT_UNITS + PLATFORM_HEIGHT_MARGIN_UNITS

    # Lowered-platform band for the reduced-height non-stair variant.
    RAMP_PLATFORM_MIN: int = RAMP_PLATFORM_HEIGHT_UNITS - PLATFORM_HEIGHT_MARGIN_UNITS
    RAMP_PLATFORM_MAX: int = RAMP_PLATFORM_HEIGHT_UNITS + PLATFORM_HEIGHT_MARGIN_UNITS
    ...
    # Obstacle marker value (kept away from the platform height band)
    OBSTACLE_MARKER: int = STAIR_PLATFORM_HEIGHT_UNITS + 100

    # Extended platform range for edge detection.
    PLATFORM_EDGE_MIN: int = STAIR_PLATFORM_HEIGHT_UNITS - PLATFORM_EDGE_MARGIN_UNITS
    PLATFORM_EDGE_MAX: int = STAIR_PLATFORM_HEIGHT_UNITS + PLATFORM_EDGE_MARGIN_UNITS

    # Extended lowered-platform range for edge detection.
    RAMP_PLATFORM_EDGE_MIN: int = RAMP_PLATFORM_HEIGHT_UNITS - PLATFORM_EDGE_MARGIN_UNITS
    RAMP_PLATFORM_EDGE_MAX: int = RAMP_PLATFORM_HEIGHT_UNITS + PLATFORM_EDGE_MARGIN_UNITS
...
class StairConfig:
    ...
    NUM_STEPS: int = DEFAULT_STAIR_NUM_STEPS

    # Original and scaled stair rise in meters
    BASE_STEP_HEIGHT_METERS: float = DEFAULT_STAIR_STEP_HEIGHT_METERS
    HEIGHT_SCALE: float = STAIR_HEIGHT_SCALE
    STEP_HEIGHT_METERS: float = STAIR_STEP_HEIGHT_METERS
    ...
    def total_height_meters(self) -> float:
        """Total staircase rise / platform height in meters."""
        return self.NUM_STEPS * self.STEP_HEIGHT_METERS

    @property
    def total_height_units(self) -> int:
        """Total staircase rise / platform height in discretized units."""
        return int(round(self.total_height_meters / VERTICAL_SCALE))
...
def is_valid_platform(height: int) -> bool:
    ...
    return (
        THRESHOLDS.PLATFORM_MIN <= height <= THRESHOLDS.PLATFORM_MAX
        or THRESHOLDS.RAMP_PLATFORM_MIN <= height <= THRESHOLDS.RAMP_PLATFORM_MAX
    )  # 同时兼容常规楼梯平台和降高后的斜坡平台。
