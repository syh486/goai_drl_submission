...
class Model1OptVqVaeView(nn.Module):
    def __init__(self, config: Model1OptVqVaeConfig | None = None):
        super().__init__()
        self.config = config or Model1OptVqVaeConfig()
...
class Model1OptVqVae(nn.Module):
    def __init__(
        self,
        config: Model1OptVqVaeConfig | None = None,
        share_view_weights: bool = True,
    ):
        super().__init__()
        self.config = config or Model1OptVqVaeConfig()
        self.share_view_weights = share_view_weights
        self.front_model = Model1OptVqVaeView(config=self.config)
        if share_view_weights:
            self.rear_model = self.front_model
        else:
            self.rear_model = Model1OptVqVaeView(config=self.config)
