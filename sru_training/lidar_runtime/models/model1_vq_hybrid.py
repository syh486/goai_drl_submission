...
class Model1VqHybridView(nn.Module):
    def __init__(self, config: Model1VqHybridConfig | None = None):
        super().__init__()
        self.config = config or Model1VqHybridConfig()
...
class Model1VqHybrid(nn.Module):
    def __init__(
        self,
        config: Model1VqHybridConfig | None = None,
        share_view_weights: bool = True,
    ):
        super().__init__()
        self.config = config or Model1VqHybridConfig()
        self.share_view_weights = share_view_weights
        self.front_model = Model1VqHybridView(config=self.config)
        if share_view_weights:
            self.rear_model = self.front_model
        else:
            self.rear_model = Model1VqHybridView(config=self.config)
