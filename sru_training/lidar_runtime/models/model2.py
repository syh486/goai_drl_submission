...
class Model2VaeSampler(nn.Module):
...
class _Model2EncoderFpn(nn.Module):
...
class Model2DepthEncoder(_Model2EncoderFpn):
...
class Model2DepthDecoder(nn.Module):
...
class Model2OriginalVaenet(nn.Module):
    def __init__(self, latent_dim: int = 64):
        super().__init__()
        self.depth_encoder = Model2DepthEncoder(latent_dim)
        self.vae_sampler = Model2VaeSampler(latent_dim, latent_dim)
        self.depth_decoder = Model2DepthDecoder(latent_dim)
...
class Model2View(nn.Module):
    def __init__(
        self,
        config: Model2Config | None = None,
        ...
        self.sru_vae = Model2OriginalVaenet(latent_dim=self.config.pretrained_latent_channels)
    ...
    def project_metric_reconstruction(self, reconstruction_logits: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        metric_reconstruction = reconstruction_logits
        clipped_reconstruction = torch.clamp(
            metric_reconstruction,
            min=self.config.output_min_m,
            max=self.config.output_max_m,
        )
        return clipped_reconstruction, clipped_reconstruction
