...
class UniformThresholdVelocityCommand(UniformVelocityCommand):
    ...
    def _resample_command(self, env_ids: Sequence[int]):
        ...
        # Official M20 zeros small planar commands before applying its
        # fixed-proportion single-axis overrides.  Default zero keeps the
        # existing A/B/C sampler unchanged.
        if self.cfg.lin_vel_deadzone > 0.0:
            planar_norm = torch.linalg.norm(self.vel_command_b[env_ids, :2], dim=1)
            self.vel_command_b[env_ids, :2] *= (planar_norm > self.cfg.lin_vel_deadzone).unsqueeze(1)
...
class UniformVelocityCommandMultiSampling(UniformVelocityCommand):
    ...
    def __init__(
        ...
        # Preserve the configured full ranges for fixed-proportion reference
        # samples.  Command curricula may later narrow ``cfg.ranges``.
    ...
    def _resample_command(self, env_ids: Sequence[int]):
        ...
        # Match the official M20 sampler: discard small planar commands before
        # applying fixed-proportion zero/single-axis overrides.  The default is
        # zero, so the existing A/B/C command distribution is unchanged.
        if self.cfg.lin_vel_deadzone > 0.0:
            planar_norm = torch.linalg.norm(self.vel_command_b[env_ids, :2], dim=1)
            self.vel_command_b[env_ids, :2] *= (
                planar_norm > self.cfg.lin_vel_deadzone
            ).unsqueeze(1)
        ...
        self._apply_fixed_proportion_samples(env_ids)

        self.vel_command_b_buffer[env_ids] = self.vel_command_b[env_ids].clone()

        self._set_zero_command_for_beginning_steps()

    def _apply_fixed_proportion_samples(self, env_ids: torch.Tensor) -> None:
        """Apply optional official-style zero and single-axis command samples.

        Defaults are all zero, so the existing A/B/C tasks retain their exact
        sampler.  Reference tasks opt in through their command config.
        """

        proportions = (
            float(self.cfg.rel_zero_vel_envs),
            float(self.cfg.rel_only_lin_y_envs),
            float(self.cfg.rel_only_lin_x_envs),
            float(self.cfg.rel_only_ang_z_envs),
        )
        if sum(proportions) <= 0.0:
            return
        if sum(proportions) > 1.0 + 1.0e-6:
            raise ValueError(f"fixed command proportions exceed one: {proportions}")

        slots = torch.rand(env_ids.numel(), device=self.device)
        lower = 0.0
        masks = []
        for proportion in proportions:
            upper = lower + proportion
            masks.append((slots >= lower) & (slots < upper))
            lower = upper

        zero_ids, only_y_ids, only_x_ids, only_yaw_ids = (env_ids[mask] for mask in masks)
        if zero_ids.numel() > 0:
            self.vel_command_b[zero_ids] = 0.0
        if only_y_ids.numel() > 0:
            self.vel_command_b[only_y_ids] = 0.0
            self.vel_command_b[only_y_ids, 1].uniform_(*self._full_ranges_lin_vel_y)
        if only_x_ids.numel() > 0:
            self.vel_command_b[only_x_ids] = 0.0
            self.vel_command_b[only_x_ids, 0].uniform_(*self._full_ranges_lin_vel_x)
        if only_yaw_ids.numel() > 0:
            self.vel_command_b[only_yaw_ids] = 0.0
            self.vel_command_b[only_yaw_ids, 2].uniform_(*self._full_ranges_ang_vel_z)
