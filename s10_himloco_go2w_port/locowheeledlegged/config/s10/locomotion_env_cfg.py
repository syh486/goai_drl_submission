...
import locowheeledlegged.mdp.s10_gait_rewards as s10_gait_rewards
import locowheeledlegged.mdp.s10_observations as s10_observations
from locowheeledlegged.assets.s10_robot import (
    HIP_JOINT_NAMES as S10_HIP_JOINT_NAMES,
    LEG_JOINT_NAMES as S10_LEG_JOINT_NAMES,
    NOMINAL_BASE_HEIGHT,
    POLICY_JOINT_NAMES,
    S10_CFG as Robot_CFG,
    WHEEL_BODY_NAMES,
    WHEEL_JOINT_NAMES as S10_WHEEL_JOINT_NAMES,
)
...
LEG_JOINT_NAMES = list(S10_LEG_JOINT_NAMES)
WHEEL_JOINT_NAMES = list(S10_WHEEL_JOINT_NAMES)
JOINT_NAMES = list(POLICY_JOINT_NAMES)
HIP_JOINT_NAMES = list(S10_HIP_JOINT_NAMES)
...
class SceneCfg(InteractiveSceneCfg):
    ...
    robot_contact_sensor = ContactSensorCfg(
        prim_path="{ENV_REGEX_NS}/Robot/(base_link|.*_hipx|.*_hipy)",
        history_length=3,
        track_air_time=True,
    )
...
WHEEL_BODY_CFG = SceneEntityCfg(
    "robot",
    body_names=list(WHEEL_BODY_NAMES),
    preserve_order=True,
)


@configclass
class ClearanceConstraintRewardsCfg(RewardsCfg):
    """A: weak anatomical corridor around each wheel center."""

    wheel_lateral_clearance = RewardTermCfg(
        func=s10_gait_rewards.wheel_lateral_clearance_l2,
        weight=-0.5,
        params={
            "min_abs_y": 0.12,
            "max_abs_y": 0.32,
...
@configclass
class TargetBandConstraintRewardsCfg(RewardsCfg):
    """B: command-adaptive wheel bands calibrated from the official ONNX."""

    wheel_lateral_target = RewardTermCfg(
        func=s10_gait_rewards.wheel_lateral_target_deadband_l2,
        weight=-0.5,
        params={
            "command_name": "base_velocity",
            "target_y": (0.1650, -0.1619, 0.1664, -0.1604),
            "base_deadband": 0.025,
            "vx_deadband_gain": 0.065,
            "omega_deadband_gain": 0.18,
            "normalizer": 0.04,
            "asset_cfg": WHEEL_BODY_CFG,
        },
    )
...
@configclass
class PairGeometryConstraintRewardsCfg(RewardsCfg):
    """C: constrain axle support width and lateral center, not joint symmetry."""

    wheel_pair_geometry = RewardTermCfg(
        func=s10_gait_rewards.wheel_pair_geometry_l2,
        weight=-0.5,
        params={
            "command_name": "base_velocity",
            "target_width": (0.3269, 0.3268),
            "target_center": (0.0016, 0.0030),
            "width_base_deadband": 0.05,
            "width_vx_gain": 0.12,
            "width_omega_gain": 0.25,
            "center_base_deadband": 0.02,
            "center_vx_gain": 0.03,
            "center_omega_gain": 0.08,
            "width_normalizer": 0.05,
            "center_normalizer": 0.03,
            "asset_cfg": WHEEL_BODY_CFG,
        },
    )
...
class EventCfg:
    ...
    # The official S10 asset has a fixed, measured asymmetric COM.  Keep it
    # unchanged instead of moving the robot-specific mass distribution.
...
class TerminationsCfg:
    ...
    hip_contact = TerminationTermCfg(
        func=mdp.illegal_contact,
        params={"sensor_cfg": SceneEntityCfg("robot_contact_sensor", body_names=".*_hip.*"), "threshold": 1.0},
    )
