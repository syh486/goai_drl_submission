...
public:
    ...
    void Start(){
        std::shared_ptr<KeyboardInterface> keyboard_ptr;
        if(remote_cmd_type_ == RemoteCommandType::kKeyBoard){
            keyboard_ptr = std::make_shared<KeyboardInterface>(robot_name_);
            uc_ptr_ = keyboard_ptr;
            ...
            uc_ptr_ = std::make_shared<DdsCommandInterface>(robot_name_);
        }else{
        ...
        if (keyboard_ptr) {
            keyboard_ptr->AttachRosNode(ri_ptr_->get_node());
        }
