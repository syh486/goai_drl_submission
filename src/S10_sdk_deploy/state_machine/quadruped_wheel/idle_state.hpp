...
namespace qw {
    ...
    private:
        ...
        double last_health_warning_time_ = -1.0;
    ...
    public:
        ...
        virtual StateName GetNextStateName() {
            if (joint_normal_flag_ != 0 || imu_normal_flag_ != 0) {
                const double now = ri_ptr_->GetInterfaceTimeStamp();
                if (last_health_warning_time_ < 0.0
                    || now - last_health_warning_time_ >= 1.0) {
                    std::cout << "========== sensor warning ========>> "
                              << "joint status: " << joint_normal_flag_
                              << ", imu status: " << imu_normal_flag_
                              << std::endl;
                    last_health_warning_time_ = now;
                }
