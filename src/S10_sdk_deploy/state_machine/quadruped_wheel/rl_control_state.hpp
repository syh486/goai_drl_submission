...
namespace qw {
    ...
    public:
        ...
        bool PostureUnsafeCheck() {
            ...
            const VecXf joint_pos = ri_ptr_->GetJointPosition();
            const VecXf joint_vel = ri_ptr_->GetJointVelocity();

            // Stop issuing RL commands after a real loss of posture. Wheel
            // positions are unbounded by design, so only leg joints use the
            // position threshold here.
            constexpr float posture_limit = 65.0f / 180.0f * M_PI;
            if (!rpy.allFinite()
                || !joint_pos.allFinite()
                || !joint_vel.allFinite()
                || std::abs(rpy(0)) > posture_limit
                || std::abs(rpy(1)) > posture_limit) {
                return true;
            }

            for (int i = 0; i < joint_pos.size(); ++i) {
                if (i % 4 != 3 && std::abs(joint_pos(i)) > 3.2f) {
                    return true;
                }
                if (std::abs(joint_vel(i)) > 40.0f) {
                    return true;
                }
            }
            return false;
